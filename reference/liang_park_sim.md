...
latent profile solution in Liang and Park's Study 2, so that the
analysis can be taught reproducibly. The authors' original data are not
redistributed here.
...
Simulated from the estimated three-class solution reported in Liang, Y.,
& Park, Y. (2025). A spectrum of bystander actions: Latent profile
analysis of sexual harassment intervention behavior at work. *Journal of
Applied Psychology*. Advance online publication.
[doi:10.1037/apl0001280](https://doi.org/10.1037/apl0001280) . The
generator and the full generating-parameter tables are not distributed
with the package.
