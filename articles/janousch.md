...
`measurement = "continuous"` is all that is needed.
...
describing a subgroup. Such a solution is not interpretable, and —
importantly — more random starts will not rescue it.

The remedy is a stronger prior holding the variances away from zero. The
warning suggests a starting point of one artificial observation per
class — `variances = 4` for this four-class model — and says to raise it
if the warning persists, which on this data it does. A little more is
enough:
...
and protective factors in each country, so cross-country comparisons of
these profiles should be made cautiously.
